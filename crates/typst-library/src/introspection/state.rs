...
/// しかしTypstでは**そうはなりません**。
/// このコードを試してみると、Typstは*Variables from outside the function are read-only and cannot be modified.*というエラーメッセージを出力することが分かります。
///
/// ```typ
/// // This doesn't work!
/// #let star = 0
...
/// 値は文書中で変化するため、これは[コンテキスト]($context)が利用可能な場合にのみ使用できる*コンテキスト*関数です。
///
