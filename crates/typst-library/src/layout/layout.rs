...
/// ```
///
/// `layout`関数はコンテンツに[ブロック]($block)レベルのコンテナであることを強制するため、そのコンテナ内部でページを基準とした配置やページ区切りはできないことを注意してください。
///
...
/// ```
///
/// `layout`が提供する幅と高さは、対象ページの寸法が`{auto}`に設定されていると無限大になりうることを注意してください。
#[func]
pub fn layout(
    span: Span,
