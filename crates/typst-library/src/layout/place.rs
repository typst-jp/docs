...
///
/// [`block`]、[`box`]、[`rect`]などの任意のコンテナが親になれます。
...
/// これを避けるため、段落の途中で`place`を呼び出す場合には、それを[`box`]でラップするとよいでしょう。
/// このとき、配置とオフセットは、この大きさを持たないボックスを基準としたものになります。
...
///
/// # アクセシビリティ
/// 支援技術（AT）は、この関数が物理的にどこへ移動させたかに関係なく、配置された要素を文書中で論理的に現れる位置で常に読み上げます。
