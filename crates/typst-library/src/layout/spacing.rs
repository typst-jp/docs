...
/// 比率指定の場合は、比率指定されたそれぞれの間隔へ、行の残りの間隔がその相対比率に応じて配分されます。
///
/// # 例
=======
...
/// 要素の大きさが比率で指定された場合、各要素には、比率の総和に対する自身の比率の割合に応じた間隔が割り当てられます。
=======
/// # Fractional spacing
...
pub struct HElem {
    ...
    /// 明示的に（通常の間隔の場合は）`[#" "]`と書くか、（改行しない間隔の場合は）`[~]`と書くことで弱い間隔の隣に間隔を強制できます。
...
///
/// 間隔の大きさは絶対的な値、相対的な値、または比率で指定できます。
/// 比率指定の場合は、比率指定されたそれぞれの間隔へ、ページの残りの間隔がその相対比率に応じて配分されます。
///
/// # 例
=======
