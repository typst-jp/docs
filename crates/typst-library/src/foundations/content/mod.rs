...
///
/// ほとんどのフィールドはオプションで、必須フィールドと同様に、単一要素を設定するために要素関数へ渡せます。しかし、[setルール]($styling/#set-rules)で設定して、スコープ内の全ての要素にも適用できます。
/// オプションのフィールドは、要素関数に明示的に渡された場合にのみフィールドアクセス構文で利用でき、setルールから適用された場合は利用できません。
///
/// 各要素にはデフォルトの外観があります。しかし、[showルール]($styling/#show-rules)を使用して、その外観を完全にカスタマイズできます。showルールには要素が渡されます。showルールは要素のフィールドにアクセスし、そこから任意のコンテンツを生成できます。
///
