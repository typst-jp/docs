...
/// 引数値から戻り値への写像。
///
/// 関数名の直後に括弧で囲まれたカンマ区切りの関数の*引数*のリストを書くことにより関数を呼び出せます。
/// 加えて、通常の引数リストの後に任意の数のコンテンツブロック引数も関数に渡せます。
...
///
/// # 要素関数
/// [見出し]($heading)や[表]($table)のような、いくつかの関数は*要素*と結びついており、呼び出すとその種類に応じた要素を作成します。
...
///
/// # ユーザー定義関数
/// [letバインディング]($scripting/#bindings)を用いることで、バインディング名の後に引数リストを持ったユーザー定義関数を定義できます。
/// 引数リストには必須の位置引数、デフォルト値を持つ名前付き引数および[可変長引数]($arguments)を用いることができます。
///
/// 関数バインディングの右辺は関数本体で、ブロックか任意の式です。
/// 関数の戻り値を定義し、引数に依存させられます。
...
/// このような関数について、戻り値の型はドキュメント中で明示的に指定されていません
...
///
/// # 関数のインポート
/// 関数は、`{import}`を用いてあるファイル（[`module`]($scripting/#modules)）から別のファイルにインポートできます。
...
/// もし関数の引数が1つだけならば、引数リストの周りに括弧を付ける必要はありません。
/// 無名関数は主にshowルールで用いると便利ですが、page関数の[`footer`]($page.footer)プロパティのような、関数を引数に取る設定可能プロパティでも便利です。
///
/// ```example
/// #show "once?": it => [#it #it]
/// once?
...
///
/// # 関数の純粋性に関する注意
/// Typstにおいて関数は全て*純粋*です。
/// これは同じ引数からは常に同じ結果が返ってくることを意味します。
/// 純粋関数は2回目の呼び出し時に別の値を生成するために何かを「記憶」できません。
///
