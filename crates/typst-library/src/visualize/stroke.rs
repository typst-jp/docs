...
/// 線がどのように描画されるかを定義します。
///
/// ストロークはペイント（単一の色またはグラデーション）、太さ、ラインキャップ、線結合、マイター制限、および破線パターンを持ちます。
...
impl Stroke {
    ...
    pub fn construct(
        ...
        /// 突出した鋭い折り返しがマイター接続の代わりにベベルによってレンダリングされる数値を指定します。
        /// 数値が高いほどより鋭い角がベベルではなくマイター接続されます。`join`フィールドが`{"miter"}`の場合のみ有効です。
        ///
        /// 厳密には、マイター制限（miter limit）とはコーナーの突き出た長さとストロークの太さの比の最大値です。
        ///
...
#[derive(Debug, Copy, Clone, Eq, PartialEq, Hash, Cast)]
pub enum LineJoin {
    /// 切片は鋭いエッジで接続されます。マイター制限を超える鋭い折れ曲がりは代わりにベベルによって接続します。
    Miter,
    /// 切片は円形のコーナーで接続されます。
    Round,
...
pub struct FixedStroke {
    ...
    /// マイター制限。`tiny-skia`と同様にデフォルトは4.0です。
