...
現在は、PDF/AとPDF/UAを同時に選択することはできません。

## PDFバージョン
Typstは5つの異なるPDFバージョンをサポートしています：1.4、1.5、1.6、1.7（デフォルト）、および2.0。
...
これらのチェックにより、アクセシビリティのベストプラクティスが順守されているかが確認されます。
...
- 独自の構造を定義するのではなく、必要に応じてTypstのセマンティック要素（[見出し]($heading)、[図表]($figure)、[リスト]($list)など）を使用していること。
  これにより、その構造が文書内で果たす役割をTypstが認識し（エクスポート時に反映する）ことができるようになります。
...
アクセシブルな文書を作成する場合、現在私たちは追加のチェック機能と幅広い互換性を考慮して、PDF 2.0の代わりにPDF/UA-1へのエクスポートを推奨しています。
...
- **PDF/A-2a：** Unicodeマッピング可能なレベルであるA-2uを基盤とした、_アクセシブル (accessible)_な適合レベル。
  ...
  最後に、PDF/A-2aでは[Unicodeの私用領域 (Private Use area)](https://ja.wikipedia.org/wiki/%E7%A7%81%E7%94%A8%E9%A0%98%E5%9F%9F)のコードポイントを使用することが禁止されています。
