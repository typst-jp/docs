<<<<<<< HEAD
PDFを出力する代わりに、Typstはページを直接スケーラブル・ベクター・グラフィックス（SVG）としてもレンダリングできます。
...
`[#set page(fill: none)]`を使用して背景を透明にできます。
