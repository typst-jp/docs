...
両者を区別すると、Typstはスクリーンリーダーなどの支援技術を利用する人にとって文書をアクセシブルにできます。
</div>
...
Typstはヘッダーにタイトルを挿入したいことを判断できないため、手動で指示する必要があります。
...
コンテキストを使うと、挿入された場所を確認し、直前の見出しや現在のページ番号などを調べ、その情報に応じて処理するようTypstに指示するコンテキストブロックを1つ記述できます。
