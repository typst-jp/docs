...
レポートを作成する際に、いくつかの主張を裏付ける必要があります。
参考文献を文書に追加するには、[`bibliography`]($bibliography) 関数を使用できます。
この関数は参考文献ファイルへのパスを期待しています。

Typstのネイティブな参考文献形式は[Hayagriva](https://github.com/typst/hayagriva/blob/main/docs/file-format.md)ですが、
互換性のためにBibLaTeXファイルも使用できます。
クラスメートが既に文献調査を行い、`.bib` ファイルを送ってくれたので、それを使用します。
ファイルパネルを通じてファイルをアップロードし、Typst appでアクセスできるようにします。

文書に参考文献が含まれたら、そこから引用を始めることができます。
引用はラベルへの参照と同じ構文を使用します。初めてソースを引用すると、そのソースが文書の参考文献セクションに表示されます。
...
それを独立した行にしたい場合は、開始と終了にそれぞれ一つのスペースを挿入する必要があります。
...
数式モードでは単一の文字は常にそのまま表示されます。しかし、複数の文字は記号、変数、または関数名として扱われます。
単一の文字間の乗算を暗黙的に示すためには、それらの間にスペースを挿入してください。

もし複数の文字からなる変数を表したい場合、それを引用符で囲むことができます。
...
この時点で、レポートの執筆が完了しました。
右上のダウンロードボタンをクリックしてPDFを保存しました。
しかし、レポートがあまりにも素朴に見えると感じるかもしれません。
